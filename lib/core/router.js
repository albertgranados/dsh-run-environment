...
import { applyAction, buildCommands, commandPayload, ICON_IDS } from './model.js';
...
/**
 * This package's own version, read once. It travels to the browser so the menu
 * can name the build it is running: the client half is served from a cache with
 * a revision computed at harness boot, so "which build am I looking at" is a
 * real question during a support conversation.
 * @returns {string} the version, or `0.0.0` when the manifest is unreadable.
 */
export function versionOfThisPackage() {
	try {
		...
		[
			'/commands',
			{
				...
				handle: async ({ query }) => {
					...
					// `logs=1` folds each run's tail into the same answer. A console needs
					// the output and the run together, and one route that is known to work
					// beats two where one of them may be filtered somewhere in between.
					const withLogs = query.get('logs') === '1';
					...
					const listed = runs.list(workspace.directory);
					return {
						...
						defaultId,
						runs: withLogs ? runs.listWithLogs(workspace.directory, bytes) : listed,
						limits,
					};
				},
			},
		...
		[
			'/log',
			{
				...
				handle: async ({ query }) => {
					...
					const requested = Number.parseInt(query.get('bytes') ?? '', 10);
					const bytes = Number.isSafeInteger(requested) && requested > 0 ? requested : DEFAULT_LOG_BYTES;
					const output = runs.tail(run, bytes);
					// `format=json` carries the same tail under `output`: the console tab
					// renders it beside the run's state, while a bare GET stays text/plain
					// for curl and the logs.
					if (query.get('format') === 'json') return { output, run: runs.payload(run) };
					return { text: output, run: runs.payload(run) };
				},
			},
		],
	]);
