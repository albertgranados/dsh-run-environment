...
export class RunRegistry {
	...
	/**
	 * The runs of one project, each carrying its retained tail. One request then
	 * holds everything a console shows, which keeps that console on a single route.
	 * @param {string} directory - project directory.
	 * @param {number} bytes - maximum tail length per run.
	 * @returns {object[]} the payloads.
	 */
	listWithLogs(directory, bytes) {
		return this.list(directory).map((payload) => {
			const run = this.runs.get(keyOf(directory, payload.id));
			return run === undefined ? payload : { ...payload, tail: this.tail(run, bytes) };
		});
	}
