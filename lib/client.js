...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css =
			...
			'.RUNENV_eye{flex:none;display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;border-radius:5px;color:var(--dsw-alias-label-tertiary);cursor:pointer}' +
			'.RUNENV_eye:hover,.RUNENV_eye:focus-visible{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}' +
			'.RUNENV_eye[data-on=true]{color:var(--dsw-alias-label-primary)}' +
		...
		function EnvironmentButton(props) {
			...
			const rowLabel = (entry) =>
				h(
					'span',
					{ className: `${styles.row}${entry.hidden === true ? ` ${styles.dim}` : ''}` },
				...
				return h(
					...
					h(
						'span',
						{
							...
							'aria-label': t('group.reveal', { manifest: title }),
							title: t('group.reveal', { manifest: title }),
							onClick: () => {
								toggleRevealed(title);
							},
						},
						h(EyeIcon, { off: !showing }),
					),
				);
			};
		...
		const zh = {
			...
			'actions.hide': '隐藏',
			'actions.show': '取消隐藏',
			'group.reveal': '显示或隐藏 {manifest} 的隐藏命令',
			'dialog.title': '添加命令',
		...
		const en = {
			...
			'actions.hide': 'Hide',
			'actions.show': 'Unhide',
			'group.reveal': 'Show or hide the hidden commands of {manifest}',
		...
		const es = {
			...
			'group.reveal': 'Mostrar u ocultar los comandos ocultos de {manifest}',
