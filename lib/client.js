...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function EnvironmentButton(props) {
			...
			/** Detected commands grouped by the manifest that declared them. */
			const groups = [];
			for (const command of detected) {
				const title = command.manifest === '' ? t('menu.detected') : command.manifest;
				...
				h(
					...
					h(
						'span',
						{
							...
							onClick: (event) => {
								// The row itself would run the command; this handle only
								// opens its options.
				...
				actionTarget === null
					...
					: h(Menu, {
							...
							items: [
								...
								{ id: UI.edit, label: t('actions.edit') },
							],
							onSelect: (id) => {
