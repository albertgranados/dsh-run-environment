...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function RunConsole({ useTabInfo, t }) {
			...
			const status = statusOf(entry?.run ?? null);
			...
			React.useEffect(() => {
				...
				void refresh();
				// A live run is polled; a finished one is read once more when the tab
				// comes back on screen, which is what `visible` re-runs.
			...
			return h(
				...
				failure === null ? null : h('div', { className: styles.consoleFailure }, failure),
				output === ''
		...
		const zh = {
			...
			'console.stopped': '已停止',
			'console.failed': '失败',
			'console.unknown': '该运行已不在当前 harness 中',
			'console.empty': '暂无输出',
			'edit.save': '保存',
			'edit.cancel': '取消',
		};
		...
		const en = {
			...
			'console.unknown': 'This run is no longer known to the harness',
		...
		const es = {
			...
			'console.unknown': 'El harness ya no conoce esta ejecución',
