...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Read one console's output. A host older than this console answers the log
		 * route as plain text: the tail is the same, only the run's facts are missing,
		 * so the caller falls back to the command list for those rather than showing a
		 * version mismatch as a broken console.
		 * @param {string} cwd - the project directory.
		 * @param {string} id - the command id.
		 * @returns {Promise<{run: object | null, output: string, plain: boolean}>} the read; `plain` marks a text answer.
		 * @throws {Error} when the host refused the read; `status` carries its answer.
		 */
		async function readConsoleTail(cwd, id) {
			...
			if (payload !== null && typeof payload.output === 'string') {
				return { run: payload.run ?? null, output: payload.output, plain: false };
			}
			return { run: null, output: body, plain: true };
		}
		...
		function RunConsole({ useTabInfo, t }) {
			...
			const refresh = React.useCallback(async () => {
				if (cwd === '' || id === '') return;
				try {
					const read = await readConsoleTail(cwd, id);
					let run = read.run;
					if (read.plain) {
						// A host that served only the tail: the run's facts come from the
						// route every version has, so an old host costs nothing.
						const info = await call(`/commands?cwd=${encodeURIComponent(cwd)}`);
						run = (Array.isArray(info.runs) ? info.runs : []).find((candidate) => candidate.id === id) ?? null;
					}
					...
					setMissing(run === null && read.plain);
					setFailure(null);
				} catch (error) {
