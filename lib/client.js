...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css =
			...
			'.RUNENV_icon{flex:none;display:block}' +
			...
			'.RUNENV_buttonStatus{display:inline-flex;align-items:center;justify-content:center;width:15px;height:15px;flex:none;color:var(--dsw-alias-label-tertiary)}' +
			// The same grey the command list uses, so one icon reads the same in both
			// places. The status dot keeps its own colour, which is the point of it.
			'.RUNENV_main .RUNENV_icon{color:var(--dsw-alias-label-tertiary)}' +
			...
			'.RUNENV_status{display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;line-height:0;border-radius:5px;cursor:pointer}' +
		...
		/**
		 * Read one console's run and its output. It comes from the command list — the
		 * one route every part of this plugin already depends on — with `logs=1`
		 * folding each run's tail into the same answer, so a console needs no second
		 * route that something in between could filter.
		 * @param {string} cwd - the project directory.
		 * @param {string} id - the command id.
		 * @returns {Promise<{run: object | null, output: string, stale: boolean}>} the read; `stale` marks a host that cannot serve the output.
		 * @throws {Error} when the host refused the read; `status` carries its answer.
		 */
		async function readConsoleRun(cwd, id) {
			const info = await call(`/commands?logs=1&cwd=${encodeURIComponent(cwd)}`);
			const runs = Array.isArray(info.runs) ? info.runs : [];
			const run = runs.find((candidate) => candidate.id === id) ?? null;
			if (run === null) return { run: null, output: '', stale: false };
			if (typeof run.tail === 'string') return { run, output: run.tail, stale: false };
			// A host older than this console does not fold the tail into the list. Its log
			// route still serves it, as text or as JSON; when that route is unreachable as
			// well there is no output to show, and the console says which build cannot
			// serve it instead of leaving the reader guessing at an empty panel.
			const aside = await readTailAside(cwd, id);
			return { run, output: aside.text, stale: !aside.reached };
		}

		/**
		 * The tail from the route a host older than this console answers, in either
		 * shape. A failure here is not worth an error line: the console still has the
		 * run's state, which is the part it cannot guess.
		 * @param {string} cwd - the project directory.
		 * @param {string} id - the command id.
		 * @returns {Promise<string>} the tail, or an empty string.
		 */
		async function readTailAside(cwd, id) {
			try {
				const url = new URL(`${hostBase()}${ROUTE_PREFIX}/log`);
				url.searchParams.set('format', 'json');
				url.searchParams.set('cwd', cwd);
				url.searchParams.set('id', id);
				const response = await fetch(url, { headers: { accept: 'application/json, text/plain' } });
				const body = await response.text();
				if (!response.ok) return { text: '', reached: false };
				try {
					const payload = JSON.parse(body);
					return { text: typeof payload.output === 'string' ? payload.output : '', reached: true };
				} catch {
					return { text: body, reached: true };
				}
			} catch {
		...
		/** How much of a console's last read is kept, per console. */
		const CONSOLE_CACHE_LIMIT = 64000;

		/**
		 * Read what one console last showed. The Sidebar keeps the tab across a
		 * harness restart, but the run behind it does not survive one, so the last
		 * output is the only thing that can keep the tab from going blank.
		 * @param {string} key - the tab's content id.
		 * @returns {string} the cached output, or an empty string.
		 */
		function readConsoleCache(key) {
			if (key === '') return '';
			try {
				return globalThis.localStorage?.getItem(`dsh-run-environment:console:${key}`) ?? '';
			} catch {
				// Storage refused (private mode, quota): the console is still live,
				// it just will not be sticky.
				return '';
			}
		}

		/**
		 * Remember one console's output, capped so a chatty command cannot fill the
		 * store.
		 * @param {string} key - the tab's content id.
		 * @param {string} text - the output just read.
		 */
		function writeConsoleCache(key, text) {
			if (key === '') return;
			try {
				globalThis.localStorage?.setItem(`dsh-run-environment:console:${key}`, text.slice(-CONSOLE_CACHE_LIMIT));
			} catch {
				// Same as above: losing the cache is not losing the console.
			}
		}
		...
		function RunConsole({ useTabInfo, t }) {
			...
			/** The address is the console's identity, and the key of what it remembers. */
			const contentId = typeof info?.tab?.contentId === 'string' ? info.tab.contentId : '';
			const [entry, setEntry] = React.useState(() => ({ run: null, output: readConsoleCache(contentId), stale: false }));
			/** The last text written to the cache, so an unchanged poll writes nothing. */
			const cached = React.useRef(entry.output);
			/** The last read failure: its status when the host answered, and its reason. */
			...
			const refresh = React.useCallback(async () => {
				if (cwd === '' || id === '') return;
				try {
					const read = await readConsoleRun(cwd, id);
					const output = read.output;
					setEntry({ run: read.run, output, stale: read.stale });
					if (output !== cached.current) {
						cached.current = output;
						writeConsoleCache(contentId, output);
					}
					setMissing(read.run === null);
					setFailure(null);
				} catch (error) {
					...
					// Anything else — the host restarting mid-poll, a dropped connection, a
					// refusal — is a hiccup: keep the output already on screen, and keep the
					// status so the line can say which of those it was.
					setFailure({
			...
			/** Whether the reader is at the bottom, i.e. following the run. */
			const sticking = React.useRef(true);
			const outputBox = React.useRef(null);
			React.useEffect(() => {
				// A console keeps its newest line in view unless the reader scrolled up to
				// read something older; the stickiness is measured on scroll, because by
				// the time the text grows the box is already taller.
				if (sticking.current && outputBox.current !== null) {
					outputBox.current.scrollTop = outputBox.current.scrollHeight;
				}
			}, [entry]);

			/** Launch this console's command again, and follow whatever it becomes. */
			const rerun = async () => {
				setPending(true);
				setFailure(null);
				try {
					await call('/run', { method: 'POST', body: { cwd, id } });
				} catch (error) {
					setFailure({
						status: typeof error?.status === 'number' ? error.status : null,
						message: error instanceof Error ? error.message : String(error),
			...
			return h(
				...
				h(
					...
					// A console with no command to point at (a hand-made tab, or a navigation
					// that lost its params) offers neither action.
					id === ''
						? null
						: state === 'running'
							? h(
				...
				failure === null
					...
					: h(
							'div',
							{ className: styles.consoleFailure, title: failure.message },
							failure.status === null ? t('console.offline') : t('console.readFailed', { status: String(failure.status) }),
						),
				output === ''
					? h(
							'div',
							{ className: styles.consoleEmpty },
							t(missing ? 'console.goneBody' : entry?.stale === true ? 'console.staleHost' : 'console.empty'),
						)
					: h(
							'pre',
							{
								...
								onScroll: (event) => {
									...
									sticking.current = box.scrollHeight - box.scrollTop - box.clientHeight < 40;
								},
							},
							output,
						),
		...
		const zh = {
			...
			'console.readFailed': 'harness 返回 {status} —— 正在重试…',
			'console.goneBody': '下面是重启前读到的内容；该运行的输出已随 harness 一起消失。',
			'console.runAgain': '再次运行',
			'console.empty': '暂无输出',
			'console.staleHost': '该 harness 比此控制台旧，无法提供输出。请重启 harness。',
			'edit.save': '保存',
			'edit.cancel': '取消',
		};
		...
		const en = {
			...
			'console.readFailed': 'The harness answered {status} — retrying…',
			'console.goneBody': 'Below is what was read before the restart; the run\'s own output went with it.',
			'console.runAgain': 'Run again',
			'console.empty': 'No output yet',
			'console.staleHost': 'This harness is older than this console, so it cannot serve the output. Restart the harness.',
			'edit.save': 'Save',
			'edit.cancel': 'Cancel',
		};
		...
		const es = {
			...
			'console.readFailed': 'El harness respondió {status} — reintentando…',
			'console.goneBody': 'Abajo está lo leído antes del reinicio; la salida del run se fue con él.',
			'console.runAgain': 'Volver a ejecutar',
			'console.empty': 'Todavía sin salida',
			'console.staleHost': 'Este harness es más antiguo que esta consola, así que no puede servir la salida. Reinicia el harness.',
			'edit.save': 'Guardar',
