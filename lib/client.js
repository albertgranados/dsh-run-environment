...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * @returns {Promise<{run: object | null, output: string}>} the read.
		...
		async function readConsoleRun(cwd, id) {
			...
			if (run === null) return { run: null, output: '' };
			if (typeof run.tail === 'string') return { run, output: run.tail };
			// A host older than this console does not fold the tail into the list. Its log
			// route still serves it, as text or as JSON: read that, and if even that is
			// unreachable show the run without output rather than an error.
			return { run, output: await readTailAside(cwd, id) };
		}
		...
		function RunConsole({ useTabInfo, t }) {
			...
			const [entry, setEntry] = React.useState(() => ({ run: null, output: readConsoleCache(contentId) }));
			...
			return h(
				...
				output === ''
					? h('div', { className: styles.consoleEmpty }, t(missing ? 'console.goneBody' : 'console.empty'))
					: h(
							'pre',
							{
