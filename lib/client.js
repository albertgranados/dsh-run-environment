...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css =
			...
			'.RUNENV_optionLabel{display:block}' +
			...
			'.RUNENV_iconOption{display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;border:.5px solid var(--dsw-alias-border-l4);border-radius:8px;color:var(--dsw-alias-label-secondary);background:0 0;cursor:pointer}' +
			'.RUNENV_iconOption:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}' +
			'.RUNENV_iconOption[data-selected=true]{color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-label-primary)}';
		const styles = {
			split: 'RUNENV_split',
			main: 'RUNENV_main',
			...
			optionLabel: 'RUNENV_optionLabel',
			more: 'RUNENV_more',
		...
		function EnvironmentButton(props) {
			...
			if (custom.length > 0) {
				items.push({
					...
					label: t('menu.delete'),
					danger: true,
					submenu: custom.map((entry) => ({ id: `${UI.deletePrefix}${entry.id}`, label: entry.label })),
				});
			}
			...
			const menu = h(Menu, {
				...
				selection: 'fill',
				onClose: () => {
					// The list and the row options belong together: closing one closes
					// the other, and neither closes the other while it opens.
					setOpen(false);
					setActions(null);
				},
				items,
				...
				actionTarget === null
					...
					: h(Menu, {
							...
							items: [
								{
									id: UI.setDefault,
									label: optionLabel(t('actions.setDefault')),
									disabled: actionTarget.id === data.defaultId,
								},
								{ id: UI.edit, label: optionLabel(t('actions.edit')) },
							],
							onSelect: (id) => {
